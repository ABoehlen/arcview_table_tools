...
*Number*-Felder werden numerisch, *String*-Felder (auch wenn sie nur Zahlen beinhalten) lexikographisch sortiert.
...
Im dbf-Format kann ein Feldname maximal 8 Stellen lang sein. Dies ist oft zu wenig um sprechende Namen zu vergeben. Ausserdem ist es manchmal sinnvoll, Feldnamen möglichst kurz zu halten, z.B. in Verbindung mit dem Calculate-Tool (siehe unten). Daher gibt es die Möglichkeit, zusätzlich einen längeren Namen als so genannten Alias zu definieren. Dieser Name wird aber nicht in der Tabelle gespeichert, sondern nur im aktuellen ArcView-Projekt (.apr). Daher gehen die Aliase verloren, wenn eine entsprechende Tabelle in ein anderes Projekt importiert wird.
...
Dies ist die mit Abstand mächtigste und vielseitigste Funktion. Da mit frei programmierbarem Avenue-Code gearbeitet wird, eröffnen sich damit mannigfaltige Möglichkeiten. Die Inspiration dazu sowie einige der verwendeten Begriffe stammen von Franz Kottiras Commodore 64-Programm «datatool» von 2003. \[3\]
...
Da bei gewissen Berechnungen die Zellen leer sein sollen, gibt es die Möglichkeit, allfällig bestehende Daten in der betreffenden Spalte zu löschen. Dazu muss das Tool mit gedrückter Shift-Taste aufgerufen werden. Die beiden ersten Zeilen, sowie mit «Code» und «Lab» gekennzeichnete Zeilen (siehe unten) werden durch diese Funktion nicht beeinflusst.

Nebst den vom Benutzer zu definierenden Variablen stehen einige Systemvariablen zur Verfügung, die nicht durch benutzerdefinierte Variablen überschrieben werden dürfen. Andernfalls riskiert man ein fehlerhaftes Verhalten oder sogar einen Absturz. Ebenso sollten die Systemvariablen `_f` und `_z` nur in Ausdrücken oder Bedingungen, d.h. lesend verwendet werden, da sie durch die laufende Berechnung automatisch mit Werten gespiesen werden.
...
Das Zeilensetting wird in der Spalte namens «Type» vorgenommen (String, 4 Characters reichen), welche den Datentypen jeder Zeile beschreibt. Folgende Typen werden unterschieden:
- <leer>: Keine Bezeichnung steht für numerische Daten, dies ist der Normalfall
- `Lab`: Kürzel für «Label». Solche Zeilen können verwendet werden, um Zwischentitel, Leerzeilen oder Erklärungen einzufügen. Zeilen dieses Typs werden von den Berechnungen des Spaltensettings ignoriert.
- `Code`: Code-Zeilen enthalten Berechnungen, welche dem Spaltensetting ähneln. Der Unterschied ist, dass die Anweisungen nur ausgeführt werden, wenn das System beim Ausführen der Spalte diese Zeile erreicht. Für die Aufnahme des Ergebnisses der Berechnungen ist die Standardvariable `_c` zu verwenden. 
- `Res`: Kürzel für «Result». Damit das Ergebnis einer Code-Zeile auch sichtbar wird, ist eine Zeile dieses Typs einzufügen, normalerweise direkt darunter.
...
In der Spalte «Bz» werden keine Berechnungen ausgeführt, dort steht normaler Text. Daher kann dort problemlos in die mit «Res» markierte Zeile als Bezeichnung «Total» geschrieben werden. Zwischentitel und Leerzeilen können ebenfalls eingefügt werden, um die Tabelle leserlicher zu gestalten. Diese Zeilen sind mit dem Kürzel «Lab» zu kennzeichnen.
